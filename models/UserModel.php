...
class UserModel extends ModelBase {
    ...
    public function getSubmenus($mainmenu) {
        ...
        if ($perfil == 5 || $perfil == 3) {
            $consulta = $this->db->executeQue("select submenus.url_submenu, submenus.icon_submenu, submenus.nombresubmenu 
                    from perfiles_permisos, submenus where idperfil=$perfil and submenus.idsubmenu=perfiles_permisos.idsubmenu and idmenu=1");
        } else {
            $consulta = $this->db->executeQue("select submenus.url_submenu, submenus.icon_submenu, submenus.nombresubmenu 
                    from perfiles_permisos, submenus where idperfil=$perfil and submenus.idsubmenu=perfiles_permisos.idsubmenu and idmenu=$menu");
        }

        $total = $this->db->numRows($consulta);
        if ($total > 0) {
            while ($row = $this->db->arrayResult($consulta)) {
                $menus_user[] = array('urlmenu' => $row['url_submenu'],
                    'icono' => $row['icon_submenu'],
                    'nombre' => $row['nombresubmenu']);
            }
        }
