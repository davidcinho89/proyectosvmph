...
class ReportComparativeStoreBrandController extends ControllerBase {
    

    public function main(){
       $this->view->setTemplate('Reports' . DS . 'comparativereportbrandStore');
        ...
        $productostodos=$productofin = $productoini = $this->model->getcategoriaInicial();
        ...
        if(isset($_GET['productoini'])){
            $productofin = $this->model->restarcategorias($_GET['productoini'], $productoini);
        ...
        if(isset($_GET['productofin'])){
            $productofin2 = $this->model->restarcategorias2($_GET['productofin'], $productofin);
            $productoselected2= $_GET['productofin'];
        }else{
            end($productofin);
            $productofin2 = $this->model->restarcategorias2(key($productofin), $productofin); 
            reset($productofin);
        }   
        ...
        if (isset($_GET['anioini'])) {
            //Codigo en caso de que sea comparar semanas con productos
            $aniomax = date("Y");
            $aniomin = date("Y", strtotime('-1 year'));
            $anioiniselected = $_GET['anioini'];
            $aniofinselected = $_GET['aniofin'];            
            ...
            $mesiniselected = $_GET['mesini'];
            $mesfinselected = $_GET['mesfin'];
            $semanainiselected = $_GET['semanaini'];
            $semanafinselected = $_GET['semanafin'];        
            $periodo=$_GET['periodo'];  
            if($periodo=="mes"){                
                $precios = $this->model->traerVentasComparativasMesCategoria($productofin2, $tiendafin2, $anioiniselected, $aniofinselected, $mesiniselected, $mesfinselected, $semanainiselected, $semanafinselected);
            }else{
                $precios = $this->model->traerVentasComparativasSemanaCategoria($productofin2, $tiendafin2, $anioiniselected, $aniofinselected, $mesiniselected, $mesfinselected, $semanainiselected, $semanafinselected);
            }                        
        } else {
            //Rangos de anios
            $aniomax = date("Y");
        ...
        $this->view->setVars('periodo', $periodo);
