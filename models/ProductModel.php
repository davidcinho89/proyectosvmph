...
class ProductModel extends ModelBase {
    ...
    public function getProductsTodos() {
        $query = "SELECT p.idproducto, p.nombreproducto, p.referencia, p.puntos, p.iva, 
                    p.estado, p.precio, p.unidadmedida, 
                    (select count(*) as proveedores from productosproveedores ppp where ppp.idproducto=p.idproducto) as proveedores,
                    (select count(*) as ventas from ventas ppp where ppp.idproducto=p.idproducto) as ventas
                    from productos p
            where estado='activo'                
    ...
    public function getProducts($idcat, $active = false) {
        ...
        if ($idbodega) {
            if ($active) {
                $query = "SELECT p.idproducto, p.nombreproducto, p.referencia, p.puntos, p.iva, 
                    p.estado, b.preciobase, p.precio, p.unidadmedida, 
                (select count(*) as proveedores from productosproveedores ppp where ppp.idproducto=p.idproducto) as proveedores,
                (select count(*) as ventas from ventas ppp where ppp.idproducto=p.idproducto) as ventas
    ...
    public function getProduct($idpro) {
        ...
        $producto = array('id' => $row['idproducto'],
            ...
            'gramospresentacion' => $row['gramospresentacion'],
            'relacioncajas' => $row['relacioncajasproductos']);
        return $producto;
    }
    ...
    public function crearProducto() {
        ...
        $idimagen = trim($_POST['imagen']);
        $unidad = $_POST['unidad'];
        $referencia = trim($_POST['referencia']);
        $precio = str_replace(",", ".", trim($_POST['price']));
        $iva = str_replace(",", ".", trim($_POST['iva']));
        $gramos = trim($_POST['gramajerelativo']);
        $relacioncajas = trim($_POST['relacioncajas']);
        ...
        if ($this->db->executeQue("insert into productos " . "
                values($idproducto,$idcategoria,'$nombrepro',$puntos,$idimagen,$iva,'activo',$precio,$precioiva,NULL,NULL,NULL,NULL,NULL,NULL,NULL,$comisionable,'$unidad',$gramos,$relacioncajas,$referencia)")) {
    ...
    public function updateProduct() {
        ...
        if ($idbodega) {
            $query = "update productos set 
                ...
                iva=$iva, porcentajeutilidad=$comisionable, referencia=$refe, gramospresentacion=$gramos, 
                relacioncajasproductos=$relacioncajas
            ...
            $query = "update productos set 
                idcategoria=$idcategoria, nombreproducto='$nombrepro', puntos=$puntos, idimagen=$idimagen, 
                iva=$iva, precio=$precio, precioiva=$precioiva, porcentajeutilidad=$comisionable, referencia=$refe, gramospresentacion=$gramos, relacioncajasproductos=$relacioncajas
                where idproducto=$idproducto";
        }
    ...
    public function deletePro() {
        if (isset($_POST["verify"])) {
            $productoid = base64_decode(urldecode(strrev($_POST["verify"])));
            $query3 = "delete from bodegasproductos where idproducto=$productoid;
            delete from productos where idproducto=$productoid;";
            if ($this->db->executeQue($query3)) {
                $respuesta['res'] = 'si';
                $respuesta['idrow'] = $productoid;
                echo json_encode($respuesta);
            } else {
                $respuesta['res'] = 'no';
                echo json_encode($respuesta);
