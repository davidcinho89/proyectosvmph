...
class ReportPriceController extends ControllerBasePrevious {

    public function main() {
        $this->view->setTemplate('Reports' . DS . 'reportprices');
    ...
    public function getmesesAjax(){
        $anio = $_POST['anioajax'];                
        $aniomax = date("Y");
        $aniomin = date("Y", strtotime('-1 year'));
        $anioiniselected = $anio;            
        //
        if ($anioiniselected == $aniomax) {
            $mesmax1 = date("m");
        } else {
        ...
        echo json_encode(array("meses"=>$mesmax1));
    }
